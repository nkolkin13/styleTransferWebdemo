...
def aug_canvas(canvas, scale, s_iter):
    ...
    save_im = np.uint8(canvas*255)
    save_im = Image.fromarray(save_im)
    d1 = ImageDraw.Draw(save_im)
    myFont = ImageFont.truetype('/home/nick/Projects/flaskStyleTransfer/flaskr/static/DroidSansMono.ttf', 40)
    d1.text((0, 0), "Working...", font=myFont, fill =(0, 200, 0))
    save_im = np.array(save_im.getdata()).reshape(canvas.shape[0], canvas.shape[1], 3)
